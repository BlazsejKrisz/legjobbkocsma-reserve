...
export async function sendReceivedEmail(data: ReceivedEmailData): Promise<boolean> {
  ...
  const { to, customerName, venueName, startsAt, endsAt, partySize, reservationId } = data

  const date = formatDateYYYYMMDD(startsAt)
  const time = formatTimeRange(startsAt, endsAt)

  const subject = `Foglalási igény beérkezett — ${venueName}`
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f3f4f6;margin:0;padding:32px 16px;">
  <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
    <div style="background:#18181b;padding:24px 32px;">
      <p style="color:#a1a1aa;font-size:12px;margin:0 0 4px;text-transform:uppercase;letter-spacing:.08em;">Legjobbkocsma</p>
      <h1 style="color:#fff;font-size:20px;margin:0;font-weight:600;">Köszönjük a foglalási igényt!</h1>
    </div>

    <div style="padding:32px;">
      <p style="color:#374151;font-size:15px;margin:0 0 24px;line-height:1.6;">
        Kedves <strong>${customerName}</strong>,<br><br>
        Megkaptuk a foglalási igényét a <strong>${venueName}</strong> helyszínre. Munkatársaink hamarosan visszaigazolják az asztalt.
      </p>

      <div style="background:#f9fafb;border-radius:8px;padding:20px;margin-bottom:24px;">
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="color:#6b7280;font-size:11px;text-transform:uppercase;letter-spacing:.06em;padding-bottom:12px;">Helyszín</td>
            <td style="color:#111827;font-size:14px;font-weight:600;text-align:right;padding-bottom:12px;">${venueName}</td>
          </tr>
          <tr>
            <td style="color:#6b7280;font-size:11px;text-transform:uppercase;letter-spacing:.06em;padding-bottom:12px;">Dátum</td>
            <td style="color:#111827;font-size:14px;font-weight:600;text-align:right;padding-bottom:12px;">${date}</td>
          </tr>
          <tr>
            <td style="color:#6b7280;font-size:11px;text-transform:uppercase;letter-spacing:.06em;padding-bottom:12px;">Időpont</td>
            <td style="color:#111827;font-size:14px;font-weight:600;text-align:right;padding-bottom:12px;">${time}</td>
          </tr>
          <tr>
            <td style="color:#6b7280;font-size:11px;text-transform:uppercase;letter-spacing:.06em;">Vendégek száma</td>
            <td style="color:#111827;font-size:14px;font-weight:600;text-align:right;">${partySize} fő</td>
          </tr>
        </table>
      </div>

      <p style="color:#9ca3af;font-size:11px;margin:0;">Foglalás #${reservationId}</p>
    </div>
  </div>
</body>
</html>`
