...
export function OpenHoursEditor({ venueId, readOnly }: Props) {
  ...
  if (isLoading) {
    return <div className="py-8 text-center text-sm text-muted-foreground">{t.open_hours.loading}</div>
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">{t.open_hours.day}</th>
              <th className="px-3 py-2 text-center text-xs font-medium text-muted-foreground">{t.open_hours.open}</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">{t.open_hours.opens}</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">{t.open_hours.closes}</th>
            </tr>
          </thead>
          <tbody>
            {fields.map((field, i) => (
      ...
      {!readOnly && (
        <div className="flex justify-end">
          <Button type="submit" size="sm" disabled={update.isPending}>
            {update.isPending ? t.common.saving : t.open_hours.save}
          </Button>
        </div>
      )}
    </form>
